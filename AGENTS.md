...
- **All code is English — never Portuguese.** This covers identifiers (functions, variables, module attributes, map/struct keys), comments, and test names. No exceptions for new code.
- **Comments are rare and short.** Keep only comments that state a non-obvious constraint or measured fact; delete narrative, section headers, and comments that restate the next line. No comments inside test bodies.
...
- **Never `GenServer.call` a bot worker from a process that has to survive.** A worker parks inside a SINGLE message for seconds (a screen capture the broker holds) or indefinitely (`Body.perform/3` waits `:infinity`), and while parked it reads nothing else in its mailbox. The default 5s call then EXITS the caller. On 2026-08-11 the caller was the Guardian's panic corner: it died halting the catcher, so the player support and the timers still pressing keys were never stopped at all, and nobody was watching the corner any more.
