defmodule Pokex.Bots.PokemonSprites do
  @moduledoc """
  His OWN pokémon, photographed from several angles — so the bot can find it on
  screen instead of assuming where it is.

  "dá pra colocarmos na calibração para eu calibrar o meu pokémon de todos
  ângulos, para tu usar e assim sempre ter tracking de onde ele tá na tela, se
  tá onde tu espera" (Lucas, 2026-08-11). The reason it matters: he parks the
  pokémon with a middle click and the corpse sweep centres on that spot — if the
  click never landed, the sweep searches empty ground and nobody notices.

  ## A separate file, on purpose

  `~/.pokex/pokemon_sprites.json`, never the corpse library. The corpse library
  IS the Catcher's aim: a pokémon taught into it is something the bot throws
  Pokéballs at. Same machine (`Pokex.Vision.SpriteLibrary`), different file, and
  the separation is the safety.

  ## Angles are samples

  A pokémon facing north and one facing south are two palettes of the same
  creature, which is exactly what the sample list is for. Ten of them, because
  unlike a corpse (which lies still) this thing turns: four cardinals, the
  diagonals, and room for the frames an animation catches mid-step.

  The NAME is the pokémon's, matching `/time` — so the tracker can ask for the
  one that is on the field rather than for whatever scores best.
  """
