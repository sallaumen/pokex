defmodule Pokex.Sim.World do
  ...
  @default_knobs %{
    ...
    # de onde a lista de batalha o vê até o pokémon, e é de onde a intuição dos
    # 5s dele sai.
    mob_ms_per_tile: 1_200,
    ...
    pet_follow_tiles: 2,
    # …e o pokémon dele quase junto com eles: "o meu pokémon anda na mesma
    # velocidade que os inimigos na média, não precisa ter esses 200 de
    # diferença, pode deixar ele uns 50 mais rápido" (27/08). Os 380 daqui eram
    # chute meu, e a diferença que ele tinha nunca foi observada.
    pet_ms_per_tile: 1_150,
    ...
    # THE CHEAP RUNG of the support ladder, invented: how much one press of the
    # pokemon's own healing skill gives back. `PlayerSupport.Logic` documents
    # the ladder — the heal is free and works in combat, the revive is the last
    # resort — and this world modelled only the last one, which is what made
    # every hurt run look like a revive treadmill.
    heal_skill_pct: 25,
  ...
  # A corner is a NEST when his hand MEASURED a pile there: the recorded
  # `gather_ms`/`fight_ms`. The `:lure_end` mark used to count as a third
  # answer, and it is gone with the mark — a route whose corners were never
  # timed now reads as road, which is honest: nothing in it says a pile lives
  # there.
