defmodule Pokex.Perception.BattleOwnRowTest do
  ...
  describe "the picture the brain builds from it" do
    ...
    test "with no health to compare, the first unreadable row is still his" do
      assert %{rows: 1, enemies: 0, own_row_seen?: :by_position} = picture(@alone, %{own_hp: nil})
    end

    test "a health nowhere near his takes no row away by health" do
      assert %{rows: 5, enemies: 4, own_row_seen?: :by_position} = picture(@five, %{own_hp: 20})
    end

    # A REGRA MUDOU AQUI, DE PROPÓSITO. Duas linhas a 57% e ele a 57 costumavam
    # ser um empate que desistia e caía no chute posicional. Desistir era o
    # comportamento errado na tela mais perigosa: uma pilha recém-chegada está
    # toda em 100% e ele também, recém-saído do revive — e era exatamente ali
    # que o desempate se recusava a trabalhar. Medido no journal dele, 33% dos
    # descontos saíam por chute. Agora ganha a MAIS PRÓXIMA, e a folga continua
    # barrando quem está longe (o teste abaixo, com 20%).
    test "two rows at his health are decided by the closest, not by position" do
      assert %{rows: 5, enemies: 4, own_row_seen?: :by_hp} = picture(@five, %{own_hp: 57})
    end
